...
import {
  IUserProfileResult,
  getUserProfile,
} from "src/dataService/api_user_profile/get";
import {
  IUpdateUserProfileData,
  updateUserProfile,
} from "src/dataService/api_user_profile/put";
...
export default function profile(): React.ReactElement {
  ...
  const {
    data: getPofileDate,
    isLoading: isLoadingGetPofileDate,
    refetch,
  } = useQuery({
        ...
        <div className="flex flex-initial justify-center">
          <div className="w-full max-w-[800px]">
            <Form layout="vertical" form={form} onFinish={() => confirm()}>
              ...
              <Form.Item label="เลขห้อง" name="room_number">
                <Input allowClear disabled />
              </Form.Item>
              ...
              {!newPassword ? (
                <div className="mb-3">
                  <Button type="primary" onClick={() => setNewPassword(true)}>
                    เปลี่ยนรหัสผ่าน
                  </Button>
                </div>
              ) : (
                <div className="mb-3">
                  <Button danger onClick={() => setNewPassword(false)}>
                    ยกเลิกเปลี่ยนรหัสผ่าน
                  </Button>
                </div>
              )}
